...
/**
 * Contains code snippets when generating javadocs through doclets for
 * {@link ServiceBusSenderClient} and
 * {@link ServiceBusSenderAsyncClient}.
 */
public class ServiceBusSenderClient {
    /**
     * Fully qualified namespace is the host name of the Service Bus resource. It
    ...
    /**
     * Code snippet demonstrating how to create an
     * {@link ServiceBusSenderAsyncClient}.
     */
    @Test
    public void instantiateAsync() {
        // BEGIN:
        // com.azure.messaging.servicebus.servicebusasyncsenderclient.instantiation
        TokenCredential credential = new DefaultAzureCredentialBuilder().build();
    ...
    public void sendBatchAsync() {
        ...
        // BEGIN:
        // com.azure.messaging.servicebus.servicebusasyncsenderclient.createMessageBatch
        // `subscribe` is a non-blocking call. The program will move onto the next line
        // of code when it starts the
        // operation. Users should use the callbacks on `subscribe` to understand the
    ...
    /**
     * Code snippet demonstrating how to create a size-limited
     * {@link ServiceBusMessageBatch} and send it.
     *
     * @throws IllegalArgumentException if an message is too large for an empty
     *                                  batch.
     */
    @Test
    public void batchSizeLimited() {
        ...
        // BEGIN:
        // com.azure.messaging.servicebus.servicebussenderclient.createMessageBatch#CreateMessageBatchOptions
        List<ServiceBusMessage> telemetryMessages = Arrays.asList(firstMessage, secondMessage, thirdMessage);

        // Setting `setMaximumSizeInBytes` when creating a batch, limits the size of
        // that batch.
        // In this case, all the batches created with these options are limited to 256
        // bytes.
        ...
        // When the batch is full, send it then create another batch to add more mesages
        // to.
    ...
    public void batchSizeLimitedAsync() {
        ...
        // BEGIN:
        // com.azure.messaging.servicebus.servicebusasyncsenderclient.createMessageBatch#CreateMessageBatchOptionsLimitedSize
        ...
        // Sends the current batch if it is not null and not empty. If the current batch
        // is null, sets it.
            ...
            if (batch.getCount() > 0) {
                return asyncSender.sendMessages(batch).then(
                        asyncSender.createMessageBatch(options)
                                .handle((ServiceBusMessageBatch newBatch,
                                        SynchronousSink<ServiceBusMessageBatch> sink) -> {
                                    // Expect that the batch we just sent is the current one. If it is not, there's
                                    // a race
        ...
        // The sample Flux contains two messages, but it could be an infinite stream of
        // telemetry messages.
    ...
    public void sendSessionMessage() {
        // BEGIN:
        // com.azure.messaging.servicebus.servicebussenderclient.sendMessage-session
        // 'fullyQualifiedNamespace' will look similar to
        // "{your-namespace}.servicebus.windows.net"
        ServiceBusSenderClient sender = new ServiceBusClientBuilder()
                .credential(fullyQualifiedNamespace, new DefaultAzureCredentialBuilder().build())
        ...
        // Setting sessionId publishes that message to a specific session, in this case,
        // "greeting".
    ...
    public void sendSessionMessageAsync() {
        // BEGIN:
        // com.azure.messaging.servicebus.servicebussenderasyncclient.sendMessage-session
        // 'fullyQualifiedNamespace' will look similar to
        // "{your-namespace}.servicebus.windows.net"

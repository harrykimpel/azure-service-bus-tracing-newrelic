package demoProducer.src.main.java.com.example.demoProducer;com.example.demoProducer;
...
/**
 * A class for aggregating {@link ServiceBusMessage messages} into a single,
 * size-limited, batch. It is treated as a
...
public final class ServiceBusMessageBatch {
     ...
     *
     * <p>
     * This method is not thread-safe; make sure to synchronize the method access
     ...
     *
     * @return {@code true} if the message could be added to the batch;
     *         {@code false} if the event was too large to fit
     *         in the batch.
     *
     * @throws NullPointerException if {@code message} is {@code null}.
     * @throws AmqpException        if {@code message} is larger than the maximum
     *                              size of the {@link
        ...
        if (isV2) {
            ...
            if (size > maxMessageSize) {
                final IndexOutOfBoundsException cause = new IndexOutOfBoundsException(
                        String.format("Requested size (%d) exceeds the maximum allowed size (%d)", size,
                                maxMessageSize));
